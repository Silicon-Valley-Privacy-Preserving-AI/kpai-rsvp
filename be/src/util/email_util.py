...
def _build_waitlist_promotion_email(
    to_email: str,
    username: str,
    title: str,
    start_time: Optional[datetime],
    ...
    plain_lines = [
        f"Hello {username},",
        "",
        f'Great news! A spot has opened up and you have been moved from the waitlist',
        f'to a confirmed RSVP for "{title}".',
        "",
    ]
    if host:
        plain_lines.append(f"Host: {host}")
      ...
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px 36px;">
            ...
            <p style="margin:0 0 24px;font-size:15px;color:#333;
                      background:#edfbea;border-left:4px solid #27ae60;
                      padding:14px 18px;border-radius:6px;">
              &#127881; A spot has opened up and you have been <strong>moved from the waitlist
              to a confirmed RSVP</strong> for this seminar. We look forward to seeing you there!
            </p>
...
def _send_waitlist_promotion_sync(
    to_email: str,
    username: str,
    title: str,
    start_time: Optional[datetime],
    ...
    msg = _build_waitlist_promotion_email(
        to_email, username, title, start_time, end_time, location, host,
    )
    _deliver(SMTP_FROM, to_email, msg.as_string())
...
async def send_waitlist_promotion_email(
    ...
    """Notify a user that they have been promoted from waitlist to confirmed RSVP."""
    if not SMTP_HOST:
        logger.info("SMTP not configured — skipping waitlist promotion email to %s", to_email)
        return
    loop = asyncio.get_event_loop()
    try:
        await loop.run_in_executor(
            None, _send_waitlist_promotion_sync,
            to_email, username, title, start_time, end_time, location, host,
        )
        logger.info("Waitlist promotion email sent to %s", to_email)
    except Exception as exc:
        logger.error("Failed to send waitlist promotion email to %s: %s", to_email, exc)
